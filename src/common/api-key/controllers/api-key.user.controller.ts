...
import { ApiKeyUserGetGuard, ApiKeyUserUpdateGuard, ApiKeyUserUpdateResetGuard } from 'src/common/api-key/decorators/api-key.user.decorator';
import { ApiKeyCreateDoc, ApiKeyGetDoc, ApiKeyListDoc, ApiKeyResetDoc, ApiKeyUpdateDoc } from 'src/common/api-key/docs/api-key.admin.doc';
...
import { AuthJwtPublicAccessProtected, AuthJwtPayload } from 'src/common/auth/decorators/auth.jwt.decorator';
...
export class ApiKeyUserController {
  ...
  @PolicyAbilityProtected({ subject: ENUM_POLICY_SUBJECT.API_KEY, action: [ENUM_POLICY_ACTION.READ] })
  ...
  async list(
    ...
    const find: Record<string, any> = { ..._search, ...isActive };

    const apiKeys: ApiKeyEntity[] = await this.apiKeyService.findAllByUser(user, find, { paging: { limit: _limit, offset: _offset }, order: _order });
    const total: number = await this.apiKeyService.getTotalByUser(user, find);
  ...
  @PolicyAbilityProtected({ subject: ENUM_POLICY_SUBJECT.API_KEY, action: [ENUM_POLICY_ACTION.READ, ENUM_POLICY_ACTION.CREATE] })
  @AuthJwtPublicAccessProtected()
  @Post('/create')
  async create(@AuthJwtPayload('_id') user: string, @Body() body: ApiKeyCreateByUserDto): Promise<IResponse> {
    ...
    try {
      const created: IApiKeyCreated = await this.apiKeyService.createByUser(user, body);
  ...
  @PolicyAbilityProtected({ subject: ENUM_POLICY_SUBJECT.API_KEY, action: [ENUM_POLICY_ACTION.READ, ENUM_POLICY_ACTION.UPDATE] })
  @AuthJwtPublicAccessProtected()
  @RequestParamGuard(ApiKeyRequestDto)
