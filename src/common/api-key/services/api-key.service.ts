...
export class ApiKeyService implements IApiKeyService {
  ...
  async findAllByUser(user: string, find?: Record<string, any>, options?: IDatabaseFindAllOptions): Promise<ApiKeyEntity[]> {
  ...
  async findOneByActiveKeyAndUser(user: string, key: string, options?: IDatabaseFindOneOptions): Promise<ApiKeyDoc> {
    return this.apiKeyRepository.findOne<ApiKeyDoc>({ key, isActive: true, user }, options);
  }

  async existByUser(user: string, options?: IDatabaseExistOptions): Promise<boolean> {
    return this.apiKeyRepository.exists({ user }, options);
  }
  ...
  async create({ name, description, startDate, endDate, user }: ApiKeyCreateDto, options?: IDatabaseCreateOptions): Promise<IApiKeyCreated> {
  ...
  async createByUser(user: string, { name, description, startDate, endDate }: ApiKeyCreateByUserDto, options?: IDatabaseCreateOptions): Promise<IApiKeyCreated> {
    const key = await this.createKey();
    const secret = await this.createSecret();
    const hash: string = await this.createHashApiKey(key, secret);
  ...
  async createRaw({ name, description, key, secret, startDate, endDate, user }: ApiKeyCreateRawDto, options?: IDatabaseCreateOptions): Promise<IApiKeyCreated> {
    const hash: string = await this.createHashApiKey(key, secret);
  ...
  async createKey(): Promise<string> {
    return this.helperStringService.random(25, { safe: false, upperCase: true, prefix: `${this.env}_` });
  }
  ...
  async inactiveManyByEndDate(options?: IDatabaseManyOptions): Promise<boolean> {
    return this.apiKeyRepository.updateMany<ApiKeyActiveDto>({ endDate: { $lte: this.helperDateService.create() }, isActive: true }, { isActive: false }, options);
  }
}
