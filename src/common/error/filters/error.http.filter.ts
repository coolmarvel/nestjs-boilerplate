...
import { IErrorException, IErrors, IErrorsImport, IValidationErrorImport } from 'src/common/error/interfaces/error.interface';
        ...
        if (responseException.errors?.length > 0) {
          errors =
            responseException._errorType === ERROR_TYPE.IMPORT
              ? this.messageService.getImportErrorsMessage(responseException.errors as IValidationErrorImport[], { customLanguages: __customLang })
              : this.messageService.getRequestErrorsMessage(responseException.errors as ValidationError[], { customLanguages: __customLang });
        }

        if (!responseException._error) _error = typeof responseException._error !== 'string' ? JSON.stringify(responseException._error) : responseException._error;
      }
    }
