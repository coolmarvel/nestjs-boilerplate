...
import { RoleDocParamsGet, RoleDocQueryIsActive, RoleDocQueryType } from 'src/common/role/constants/role.doc.constant';
...
export function RoleListDoc(): MethodDecorator {
  return applyDecorators(
    DocPaging<RoleListSerialization>('role.list', {
      ...
      request: { queries: [...RoleDocQueryIsActive, ...RoleDocQueryType] },
      response: { serialization: RoleListSerialization },
    }),
  );
}
...
export function RoleGetDoc(): MethodDecorator {
  return applyDecorators(
    Doc<RoleGetSerialization>('role.get', {
      auth: { jwtAccessToken: true },
      request: { params: RoleDocParamsGet },
...
export function RoleCreateDoc(): MethodDecorator {
  return applyDecorators(
    Doc<ResponseIdSerialization>('role.create', {
      auth: { jwtAccessToken: true },
      response: { httpStatus: HttpStatus.CREATED, serialization: ResponseIdSerialization },
    }),
  );
}
