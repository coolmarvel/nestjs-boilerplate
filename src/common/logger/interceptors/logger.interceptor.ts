    ...
    if (context.getType() === 'http') {
      ...
      return next.handle().pipe(
        tap(async (response: Promise<Record<string, any>>) => {
          ...
          const loggerAction: ENUM_LOGGER_ACTION = this.reflector.get<ENUM_LOGGER_ACTION>(LOGGER_ACTION_META_KEY, context.getHandler());
          const loggerOptions: ILoggerOptions = this.reflector.get<ILoggerOptions>(LOGGER_OPTIONS_META_KEY, context.getHandler());

          await this.loggerService.raw({
            level: loggerOptions?.level ?? ENUM_LOGGER_LEVEL.INFO,
            action: loggerAction,
            description: loggerOptions?.description ?? `Request ${method} called, url ${originalUrl}, and action ${loggerAction}`,
            apiKey: apiKey?._id,
            user: user?._id,
            requestId: __id,
      ...
      return next.handle().pipe(
        tap(async () => {
          ...
          await this.loggerService.raw({
            level: loggerOptions.level || ENUM_LOGGER_LEVEL.INFO,
            action: loggerAction,
            description: loggerOptions.description ? loggerOptions.description : `Request ${topic} called, url ${topic}, and action ${loggerAction}`,
            requestId: key,
