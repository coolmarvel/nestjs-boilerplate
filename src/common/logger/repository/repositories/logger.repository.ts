...
@Injectable()
export class LoggerRepository extends DatabaseMongoUUIDRepositoryAbstract<LoggerEntity, LoggerDoc> {
  constructor(
    @DatabaseModel(LoggerEntity.name)
    private readonly LoggerDoc: Model<LoggerEntity>,
  ) {
    super(LoggerDoc, { path: 'apiKey', match: '_id', model: ApiKeyEntity.name });
  }
}
