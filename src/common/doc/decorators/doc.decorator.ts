...
import { ApiBearerAuth, ApiBody, ApiConsumes, ApiExtraModels, ApiHeader, ApiHeaders, ApiParam, ApiProduces, ApiQuery, ApiResponse, ApiSecurity, getSchemaPath } from '@nestjs/swagger';
import { APP_LANGUAGE } from 'src/app/constants/app.constant';
import { ENUM_API_KEY_STATUS_CODE_ERROR } from 'src/common/api-key/constants/api-key.status-code.constant';
import { ENUM_AUTH_STATUS_CODE_ERROR } from 'src/common/auth/constants/auth.status-code.constant';
import { ENUM_DOC_REQUEST_BODY_TYPE, ENUM_DOC_RESPONSE_BODY_TYPE } from 'src/common/doc/constants/doc.enum.constant';
...
export function Doc<T>(messagePath: string, options?: IDocOptions<T>): MethodDecorator {
  ...
  if (!normDoc.statusCode) normDoc.statusCode = normDoc.httpStatus;

  if (options?.request?.bodyType === ENUM_DOC_REQUEST_BODY_TYPE.FORM_DATA) {
    docs.push(ApiConsumes('multipart/form-data'));

    if (options?.request?.file?.multiple) docs.push(ApiBody({ description: 'Multiple file', type: FileMultipleDto }));
    else if (!options?.request?.file?.multiple) docs.push(ApiBody({ description: 'Single file', type: FileSingleDto }));
  } else if (options?.request?.bodyType === ENUM_DOC_REQUEST_BODY_TYPE.TEXT) docs.push(ApiConsumes('text/plain'));
  else docs.push(ApiConsumes('application/json'));

  if (options?.response?.bodyType === ENUM_DOC_RESPONSE_BODY_TYPE.FILE) docs.push(ApiProduces(ENUM_FILE_EXCEL_MIME.XLSX));
  else if (options?.response?.bodyType === ENUM_DOC_RESPONSE_BODY_TYPE.TEXT) docs.push(ApiProduces('text/plain'));
  else {
    docs.push(ApiProduces('application/json'));
    if (options?.response?.serialization) normDoc.serialization = options?.response?.serialization;
  }
  docs.push(DocDefault(normDoc));

  if (options?.request?.params) docs.push(...options?.request?.params.map((param) => ApiParam(param)));

  if (options?.request?.queries) docs.push(...options?.request?.queries.map((query) => ApiQuery(query)));
    ...
    ApiResponse({
      ...
      schema: {
        ...
        properties: {
          ...
          data: {
            type: 'array',
            items: { $ref: getSchemaPath(options.response.serialization) },
          },
        },
      },
    }),
...
export function DocDefault<T>(options: IDocDefaultOptions): MethodDecorator {
  ...
  const schema: Record<string, any> = {
    ...
    properties: {
      message: { example: options.messagePath },
      statusCode: { type: 'number', example: options.statusCode },
    },
  };
