...
export class AuthService implements IAuthService {
  ...
  async decryptAccessToken({ data }: Record<string, any>): Promise<Record<string, any>> {
    return this.helperEncryptionService.aes256Decrypt(data, this.accessTokenEncryptKey, this.accessTokenEncryptIv) as Record<string, any>;
  }
  ...
  async decryptRefreshToken({ data }: Record<string, any>): Promise<Record<string, any>> {
    return this.helperEncryptionService.aes256Decrypt(data, this.refreshTokenEncryptKey, this.refreshTokenEncryptIv) as Record<string, any>;
  }

  async createRefreshToken(payloadHashed: string | Record<string, any>, options?: IAuthRefreshTokenOptions): Promise<string> {
    return this.helperEncryptionService.jwtEncrypt(
      { data: payloadHashed },
      {
  ...
  async createPayloadAccessToken(data: Record<string, any>, options?: IAuthPayloadOptions): Promise<Record<string, any>> {
    return { ...data, loginDate: options?.loginDate ?? this.helperDateService.create() };
  }

  async createPayloadRefreshToken(_id: string, options: IAuthPayloadOptions): Promise<Record<string, any>> {
    return { _id, loginDate: options?.loginDate };
  }
