...
export function PaginationPagingPipe(defaultPerPage: number): Type<PipeTransform> {
  ...
  class MixinPaginationPagingPipe implements PipeTransform {
    ...
    async transform(value: Record<string, any>): Promise<Record<string, any>> {
      const page: number = this.paginationService.page(this.helperNumberService.create(value?.page ?? 1));
      const perPage: number = this.paginationService.perPage(this.helperNumberService.create(value?.perPage ?? defaultPerPage));
