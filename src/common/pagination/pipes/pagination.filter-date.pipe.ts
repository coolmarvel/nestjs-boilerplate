...
export function PaginationFilterDatePipe(options?: IPaginationFilterDateOptions): Type<PipeTransform> {
  ...
  class MixinPaginationFilterDatePipe implements PipeTransform {
    ...
    async transform(value: string, { data: field }: ArgumentMetadata): Promise<Record<string, Date>> {
      let date: Date = this.helperDateService.create(value);

      if (options?.time === ENUM_PAGINATION_FILTER_DATE_TIME_OPTIONS.END_OF_DAY) date = this.helperDateService.endOfDay(date);
      else if (options?.time === ENUM_PAGINATION_FILTER_DATE_TIME_OPTIONS.START_OF_DAY) date = this.helperDateService.startOfDay(date);

      this.request.__filters = { ...this.request.__filters, [field]: value };
