...
export function PaginationFilterContainPipe(options?: IPaginationFilterStringContainOptions): Type<PipeTransform> {
  ...
  class MixinPaginationFilterContainPipe implements PipeTransform {
    ...
    async transform(value: string, { data: field }: ArgumentMetadata): Promise<Record<string, { $regex: RegExp; $options: string }>> {
      if (!value) value = '';

      if (options?.case === ENUM_PAGINATION_FILTER_CASE_OPTIONS.UPPERCASE) value = value.toUpperCase();
      else if (options?.case === ENUM_PAGINATION_FILTER_CASE_OPTIONS.LOWERCASE) value = value.toUpperCase();

      if (options?.trim) value = value.trim();

      if (options?.fullMatch) return this.paginationService.filterContainFullMatch(field, value);
