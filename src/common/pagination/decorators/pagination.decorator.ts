...
export function PaginationQueryFilterInBoolean(field: string, defaultValue: boolean[]): ParameterDecorator {
  return Query(field, PaginationFilterInBooleanPipe(defaultValue));
}

export function PaginationQueryFilterInEnum<T>(field: string, defaultValue: T, defaultEnum: Record<string, any>): ParameterDecorator {
  return Query(field, PaginationFilterInEnumPipe<T>(defaultValue, defaultEnum));
}

export function PaginationQueryFilterEqual(field: string, options?: IPaginationFilterStringEqualOptions): ParameterDecorator {
  return Query(field, PaginationFilterEqualPipe(options));
}

export function PaginationQueryFilterContain(field: string, options?: IPaginationFilterStringContainOptions): ParameterDecorator {
