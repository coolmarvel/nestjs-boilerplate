...
import { IDebuggerHttpConfig, IDebuggerHttpConfigOptions, IDebuggerHttpMiddleware } from 'src/common/debugger/interfaces/debugger.interface';
...
export class DebuggerHttpMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction): Promise<void> {
    if (this.writeIntoConsole || this.writeIntoFile) this.customToken();
...
export class DebuggerHttpWriteIntoFileMiddleware implements NestMiddleware {
  ...
  private async httpLogger(): Promise<IDebuggerHttpConfig> {
    ...
    return { debuggerHttpFormat: DEBUGGER_HTTP_FORMAT, debuggerHttpOptions };
  }
...
export class DebuggerHttpWriteIntoConsoleMiddleware implements NestMiddleware {
  ...
  private async httpLogger(): Promise<IDebuggerHttpConfig> {
    return { debuggerHttpFormat: DEBUGGER_HTTP_FORMAT };
  }
