...
export class AwsS3Service implements IAwsS3Service {
  ...
  async getItemInBucket(filename: string, path?: string): Promise<Readable | ReadableStream<any> | Blob> {
    if (path) path = path.startsWith('/') ? path.replace('/', '') : `${path}`;

    const key: string = path ? `${path}/${filename}` : filename;
    const command: GetObjectCommand = new GetObjectCommand({ Bucket: this.bucket, Key: key });
  ...
  async putItemInBucket(filename: string, content: string | Uint8Array | Buffer | Readable | ReadableStream | Blob, options?: IAwsS3PutItemOptions): Promise<AwsS3Serialization> {
  ...
  async deleteItemInBucket(filename: string): Promise<void> {
    const command: DeleteObjectCommand = new DeleteObjectCommand({ Bucket: this.bucket, Key: filename });
  ...
  async createMultiPart(filename: string, options?: IAwsS3PutItemOptions): Promise<AwsS3MultipartSerialization> {
    let path: string = options?.path;
    const acl: string = options?.acl ? options.acl : 'public-read';
  ...
  async uploadPart(path: string, content: UploadPartRequest['Body'] | string | Uint8Array | Buffer, uploadId: string, partNumber: number): Promise<AwsS3MultipartPartsSerialization> {
  ...
  async completeMultipart(path: string, uploadId: string, parts: CompletedPart[]): Promise<void> {
