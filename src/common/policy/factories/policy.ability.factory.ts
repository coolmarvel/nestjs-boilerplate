...
import { IPolicyAbility, IPolicyRule, IPolicyRuleAbility, PolicyHandler } from 'src/common/policy/interfaces/policy.interface';
...
export class PolicyAbilityFactory {
  defineAbilityFromRole({ type, permissions }: RoleDoc) {
    ...
    if (type === ENUM_ROLE_TYPE.SUPER_ADMIN) can(ENUM_POLICY_ACTION.MANAGE, 'all');
  ...
  mappingAbility({ subject, action }: IPolicyRule): IPolicyRuleAbility[] {
    return action.map((val) => ({ action: val, subject })).flat(1);
  }
}
