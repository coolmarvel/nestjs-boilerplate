...
import { IDatabaseCreateOptions, IDatabaseFindAllOptions, IDatabaseFindOneOptions, IDatabaseOptions, IDatabaseManyOptions } from 'src/common/database/interfaces/database.interface';
...
export class SettingService implements ISettingService {
  ...
  async findAll(find?: Record<string, any>, options?: IDatabaseFindAllOptions): Promise<SettingEntity[]> {
  ...
  async updateValue(repository: SettingDoc, { type, value }: SettingUpdateValueDto): Promise<SettingDoc> {
  ...
  async getValue<T>(setting: SettingDoc): Promise<T> {
    ...
    else if (setting.type === ENUM_SETTING_DATA_TYPE.NUMBER && this.helperNumberService.check(setting.value)) return this.helperNumberService.create(setting.value) as any;
  ...
  async getMaintenance(): Promise<boolean> {
    const setting: SettingDoc = await this.settingRepository.findOne<SettingDoc>({ name: 'maintenance' });
