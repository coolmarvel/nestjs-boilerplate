...
export class FileValidationPipe<T> implements PipeTransform {
  ...
  async transform(value: IFileExtract<T> | IFileExtract<T>[]): Promise<IFileExtract<T> | IFileExtract<T>[]> {
    if (!value) return;
  ...
  async transformMerge(value: IFileExtract, classTransform: T[]): Promise<IFileExtract<T>> {
    return { ...value, dto: classTransform };
  }

  async transformExtract(classDtos: ClassConstructor<T>, extract: Record<string, any>[]): Promise<T[]> {
