import { PipeTransform, Injectable, PayloadTooLargeException, Scope, Inject } from '@nestjs/common';
...
export class FileSizeImagePipe implements PipeTransform {
  ...
  async validate(size: number): Promise<void> {
    const maxSizeInBytes = this.request.__customMaxFileSize ? this.helperFileService.convertToBytes(this.request.__customMaxFileSize) : this.maxSize;
