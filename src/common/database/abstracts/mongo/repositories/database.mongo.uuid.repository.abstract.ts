...
export abstract class DatabaseMongoUUIDRepositoryAbstract<Entity, EntityDocument> extends DatabaseBaseRepositoryAbstract<EntityDocument> {
  ...
  async delete(repository: EntityDocument & Document<string>): Promise<EntityDocument> {
    return repository.deleteOne();
  }

  async softDelete(repository: EntityDocument & Document<string> & { deletedAt?: Date }): Promise<EntityDocument> {
    repository.deletedAt = new Date();
    return repository.save();
  }

  async restore(repository: EntityDocument & Document<string> & { deletedAt?: Date }): Promise<EntityDocument> {
