...
export class DatabaseOptionsService implements IDatabaseOptionsService {
  ...
  createOptions(): MongooseModuleOptions {
    ...
    let uri = `${host}`;
    if (database) uri = `${uri}/${database}${options}`;

    if (env !== ENUM_APP_ENVIRONMENT.PRODUCTION) mongoose.set('debug', debug);

    const mongooseOptions: MongooseModuleOptions = { uri, serverSelectionTimeoutMS: 5000, autoCreate: true };

    if (user && password) mongooseOptions.auth = { username: user, password: password };
