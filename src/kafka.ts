...
export default async function (app: NestApplication) {
  ...
  const consumerGroup: string = configService.get<string>('kafka.consumer.groupId');

  const consumer: ConsumerConfig = configService.get<ConsumerConfig>('kafka.consumer');
  const allowAutoTopicCreation: boolean = configService.get<boolean>('kafka.allowAutoTopicCreation');
  ...
  if (enable) {
    app.connectMicroservice<MicroserviceOptions>({
      ...
      options: {
        ...
        subscribe,
        consumer: { ...consumer, allowAutoTopicCreation },
      },
    });

    await app.startAllMicroservices();

    logger.log(`Kafka server ${clientId} connected on brokers ${brokers.join(', ')}`, 'NestApplication');
