...
export default registerAs(
  'kafka',
  (): Record<string, any> => ({
    clientId: process.env.KAFKA_CLIENT_ID || 'KAFKA_COOLMARVEL',
    brokers: process.env.KAFKA_BROKERS ? process.env.KAFKA_BROKERS.split(',') : ['localhost:9092'],

    // consumer
    consumerEnable: Boolean(process.env.KAFKA_CONSUMER_ENABLE) === true,
    consumer: {
      groupId: process.env.KAFKA_CONSUMER_GROUP || 'nestjs.coolmarvel',
    ...
    consumerSubscribe: { fromBeginning: false },
    ...
    // admin
    admin: { clientId: process.env.KAFKA_ADMIN_CLIENT_ID || 'KAFKA_ADMIN_COOLMARVEL', defaultPartition: 3 },
  }),
);
