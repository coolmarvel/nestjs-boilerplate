...
@Injectable()
export class KafkaAdminService implements IKafkaAdminService, OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.kafkaOptions = { clientId: this.clientId, brokers: this.brokers };

    this.defaultPartition = this.configService.get<number>('kafka.admin.defaultPartition');
  ...
  async getAllTopicUnique(): Promise<string[]> {
    return [...new Set(await this.getAllTopic())].filter((val) => val !== '__consumer_offsets');
  }
  ...
  async createTopics(): Promise<boolean> {
    ...
    for (const topic of this.topics) {
      ...
      const replicationFactor: number = topic.replicationFactor && topic.replicationFactor <= this.brokers.length ? topic.replicationFactor : this.brokers.length;
      if (!currentTopic.includes(topic.topic)) data.push({ topic: topic.topic, numPartitions: partition, replicationFactor: replicationFactor });

      if (!currentTopic.includes(topic.topicReply)) data.push({ topic: topic.topicReply, numPartitions: partition, replicationFactor });
    }

    if (data.length > 0) await this.admin.createTopics({ waitForLeaders: true, topics: data });
  ...
  async deleteTopics(): Promise<boolean> {
    ...
    for (const topic of this.topics) {
      if (currentTopic.includes(topic.topic)) data.push(topic.topic);

      if (currentTopic.includes(topic.topicReply)) data.push(topic.topicReply);
    }

    if (data.length > 0) await this.admin.deleteTopics({ topics: data });
