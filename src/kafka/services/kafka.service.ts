...
import { IKafkaMessage, IKafkaProducerMessageOptions, IKafkaProducerSendMessageOptions } from 'src/kafka/interfaces/kafka.interface';
...
export class KafkaService implements IKafkaService, OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    Object.values(ENUM_KAFKA_TOPICS).forEach((topic) => this.clientKafka.subscribeToResponseOf(topic));
  ...
  async produceSend<T, N>(topic: string, data: T, options?: IKafkaProducerSendMessageOptions): Promise<IKafkaMessage<N> | N> {
    ...
    const send = await firstValueFrom(this.clientKafka.send<any, string>(topic, JSON.stringify(message)).pipe(timeout(this.timeout)));
  ...
  produceEmit<T, N>(topic: string, data: T, options?: IKafkaProducerMessageOptions): Observable<N> {
    ...
    return this.clientKafka.emit<any, string>(topic, JSON.stringify(message)).pipe(timeout(this.timeout));
  }

  async produceSendSequential<T, N = any>(topic: string, data: T, options?: IKafkaProducerSendMessageOptions): Promise<N> {
    const message: IKafkaMessage<T> = {
  ...
  async commitOffsets(context: KafkaContext): Promise<void> {
    ...
    return this.clientKafka.commitOffsets([{ topic: kafkaTopic, partition: kafkaPartition, offset }]);
  }
}
